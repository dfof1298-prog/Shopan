...
def get_curl_session(proxy_str=None, timeout=45):
    """إنشاء جلسة curl_cffi مع إعدادات محسنة"""
    session = CurlSession(
        timeout=timeout,
...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    ...
    try:
        ...
        if not variant_id:
            # جلب المنتجات باستخدام curl_cffi
            try:
                ...
                if resp.status_code != 200:
                    session.close()
                    return False, f"<b>Site Error! Status: {resp.status_code}</b>", gateway, total_price, currency
                
                text = resp.text
                if "shopify" not in text.lower():
                    session.close()
                    return False, "<b>Not Shopify!</b>", gateway, total_price, currency

                result = resp.json().get('products', [])
                if not result:
                    session.close()
                    return False, "<b>No Products!</b>", gateway, total_price, currency
                ...
                if isinstance(min_product, dict) and min_product.get('variant_id'):
                    ...
                    return False, "<b>No Valid Products</b>", gateway, total_price, currency
                    
            except Exception as e:
                return False, f"error: {str(e)}", gateway, total_price, currency

        # استخدام curl_cffi للطلبات الأولية
        ...
        # استخدام curl_cffi للطلبات GraphQL
        curl_session = get_curl_session(proxy_str)
...
def shopify_checker():
    try:
        # التحقق من الذاكرة قبل المعالجة
        ...
        # استخدام ThreadPoolExecutor لإدارة الحلقات بشكل صحيح
...
# إضافة نقطة نهاية لمراقبة الصحة
...
if __name__ == "__main__":
    # تثبيت psutil إذا لم يكن مثبتاً
    try:
        import psutil
    except ImportError:
