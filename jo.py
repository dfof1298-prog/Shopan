...
async def make_graphql_request_with_captcha_handling(
    ...
    for attempt in range(max_retries + 1):
        try:
            # زيادة وقت المهلة للطلبات الكبيرة
            ...
            try:
                ...
                if attempt < max_retries:
                    ...
                    continue
                return response, f"Invalid JSON response: {response_text[:200]}", False
...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    # استخدام Semaphore للتحكم في الطلبات المتزامنة
    async with request_semaphore:
        ...
        try:
            ...
            connector = aiohttp.TCPConnector(ssl=False, limit=5, enable_cleanup_closed=True)
            ...
            async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
                ...
                try:
                    ...
                    # محاولة قراءة الاستجابة مرة أخرى إذا كانت مشوهة
                ...
                # محاولة submit مع إعادة المحاولة
                response, text, captcha_solved = await make_graphql_request_with_captcha_handling(
                    session, graphql_url, params, headers, submit_json_data, checkout_url, max_retries=3
                )
...
def shopify_checker():
    try:
        ...
        try:
            ...
            loop.close()
            # تنظيف الذاكرة بعد إغلاق الحلقة
            gc.collect()
