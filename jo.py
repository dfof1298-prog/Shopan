...
# الحد الأقصى للطلبات المتزامنة - تقليل الضغط
MAX_CONCURRENT_REQUESTS = 3
request_semaphore = Semaphore(MAX_CONCURRENT_REQUESTS)

# إعدادات المهلة
REQUEST_TIMEOUT = 90  # زيادة المهلة
CONNECTION_LIMIT = 3  # تقليل عدد الاتصالات المتزامنة
...
async def make_graphql_request_with_captcha_handling(
    ...
    checkout_url, max_retries=3, solve_captcha=True
):
        ...
        try:
            ...
            try:
                ...
                if attempt < max_retries:
                    ...
                    await asyncio.sleep(wait_time)
                    continue
                return response, f"Invalid JSON: {response_text[:200]}", False
            ...
            if attempt < max_retries:
                ...
                await asyncio.sleep(wait_time)
                continue
            return None, f"Timeout after {max_retries} retries", False
...
async def fetch_products(domain, proxy_str=None):
    try:
        ...
        connector = aiohttp.TCPConnector(ssl=False, limit=CONNECTION_LIMIT)
        timeout = aiohttp.ClientTimeout(total=20)
        
        proxy = parse_proxy(proxy_str) if proxy_str else None
        
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            async with session.get(f"{domain}/products.json", proxy=proxy, timeout=20) as resp:
                if resp.status != 200:
        ...
        try:
            ...
            async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
                ...
                response, resp_text, captcha_solved = await make_graphql_request_with_captcha_handling(
                    session, graphql_url, params, headers, json_data, checkout_url, max_retries=3
                )
                ...
                try:
                    ...
                    try:
                        ...
                        if response and response.status == 200:
                            ...
                            return False, f"Invalid JSON after retry: {str(e)}", gateway, total_price, currency
                    except:
                ...
                response, text, captcha_solved = await make_graphql_request_with_captcha_handling(
                    session, graphql_url, params, headers, submit_json_data, checkout_url, max_retries=4
                )
            ...
            logger.error(f"Error in process_card: {traceback.format_exc()}")
...
def shopify_checker():
    try:
        ...
        logger.error(f"Error in shopify_checker: {traceback.format_exc()}")
        return jsonify({
            "error": str(e),
            "status": False,
            "Gateway": "UNKNOWN",
