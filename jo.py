...
async def make_graphql_request_with_captcha_handling(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            try:
                ...
                return response, response_text, False # Successful response and JSON parse
            except json.JSONDecodeError:
                if attempt < max_retries:
                    await asyncio.sleep(1) # Short delay for JSON decode errors
                    continue
                else:
                    return None, f"Invalid JSON response after {max_retries + 1} attempts: {response_text}", False
    ...
    return None, "Max retries reached without a valid response.", False
