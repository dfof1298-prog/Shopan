...
async def fetch_products(domain, proxy_str=None):
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            async with session.get(f"{domain}/products.json", proxy=proxy, timeout=15) as resp:
                if resp.status != 200:
...
def get_curl_session(proxy_str=None, timeout=30):
...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    ...
    try:
        ...
        # استخدام aiohttp للطلبات المتزامنة
        connector = aiohttp.TCPConnector(ssl=False, limit=20, ttl_dns_cache=300, force_close=True)
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            # طلب توكن الدفع باستخدام curl_cffi
            curl_session = get_curl_session(proxy_str)
            ...
            try:
                response = curl_session.post('https://checkout.pci.shopifyinc.com/sessions', json=payload, headers=vault_headers)
