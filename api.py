...
async def make_graphql_request_with_captcha_handling(session, graphql_url, params, headers, json_data, checkout_url, max_retries=1, solve_captcha=True, proxy=None):
    original_variables = json_data.get('variables', {}).copy()
    
    for attempt in range(max_retries + 1):
        try:
            response = curl_post(graphql_url, params=params, headers=headers, json=json_data)
...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    ...
    try:
        ...
        if True: # Using curl_cffi
            ...
            response = curl_post(url=checkout, allow_redirects=True, headers=checkout_headers, proxy=proxy)
            ...
            for i in range(4):
                response, final_text, captcha_solved = await make_graphql_request_with_captcha_handling(
                    None, graphql_url, params, headers, poll_json_data, checkout_url, max_retries=1, proxy=proxy
                )
