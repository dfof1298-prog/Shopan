...
async def make_graphql_request_with_captcha_handling(
    ...
    for attempt in range(max_retries + 1):
        try:
            proxy = parse_proxy(session._proxy_str if hasattr(session, '_proxy_str') else None)
            async with session.post(graphql_url, params=params, headers=headers, json=json_data, proxy=proxy) as response:
                ...
                # Check for JSON validity to avoid "Invalid JSON" errors later
                try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            async with session.get(f"{domain}/products.json", proxy=proxy, timeout=15) as resp:
                ...
                if is_captcha_required(text):
                    return False, "<b>Cloudflare/Captcha on Site!</b>"
                ...
                try:
                    ...
                    return False, "<b>Invalid JSON in products!</b>"
                if not result:
...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None):
    ...
    try:
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            session._proxy_str = proxy_str # Store for use in GraphQL helper
            url = ourl
...
def shopify_checker():
    try:
        ...
        try:
            # Avoid creating new event loop per request to prevent memory leaks
            success, message, gateway, price, currency = asyncio.run(
                process_card_async(cc, mes, ano, cvv, site, variant_id, proxy_str)
            )
        except Exception as e:
            return jsonify({"error": f"Async error: {str(e)}", "status": False}), 500
